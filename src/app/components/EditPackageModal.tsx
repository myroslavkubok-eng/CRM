...
import type { PackageDeal } from './ServicesTab';
...
interface EditPackageModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (packageData: UpdatedPackage) => void;
  packageDeal: PackageDeal | null;
}

export function EditPackageModal({ isOpen, onClose, onSubmit, packageDeal }: EditPackageModalProps) {
  ...
  useEffect(() => {
    if (isOpen && packageDeal) {
      setPackageName(packageDeal.name);
      setDescription(packageDeal.description);
      setSelectedServices(packageDeal.services);
      setValidityDays(packageDeal.validityDays);
      
      // Calculate discount type and value from existing package
      const originalPrice = packageDeal.originalPrice;
      const discountedPrice = packageDeal.discountedPrice;
      const discountAmount = originalPrice - discountedPrice;
      const discountPercentage = packageDeal.discount;
      
      // Default to percentage if discount exists
  ...
  if (!isOpen || !packageDeal) return null;
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 p-6 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-purple-100">Update package deal details</p>
            </div>
          </div>
          <button
            onClick={handleClose}
